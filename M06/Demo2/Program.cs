    ...
    public class Program
    {
        ...
        public async Task GetStartedDemoAsync()
        {
            ...
            while (true)
            {
                ...
                await this.QueryItemsAsync("SELECT * FROM c ORDER BY c.OrderCustomer.Name ASC");
                await this.QueryItemsAsync("SELECT * FROM c WHERE c.OrderCustomer.Name IN ('UpperLevel','Level4you') ");
                await this.QueryItemsAsync("SELECT * FROM c WHERE c.OrderCustomer.Name IN ('UpperLevel','Level4you') and c.OrderCustomer.IsActive = true");
                await this.QueryItemsAsync("SELECT * FROM c WHERE c.OrderCustomer.Name IN ('UpperLevel','Level4you') and c.OrderCustomer.IsActive = true ORDER BY c.OrderCustomer.Name ASC");
        ...
        private async Task QueryItemsAsync(string sqlQueryText)
        {
            ...
            while (queryResultSetIterator.HasMoreResults)
            {
                ...
                Console.WriteLine("Select orders. Operation consumed {0} ms and {1} RUs.\n", sp.ElapsedMilliseconds, currentResultSet.RequestCharge);
            } 
        }
    }
}
