...
async function queryItems(container) {
    ...
    var querySpec = {
        query: 'SELECT * from items WHERE items.name = @name',
        parameters: [{ name: '@name', value: 'Cosmos DB' }]
    };
...
init(client, process.env.databaseId, process.env.containerId, process.env.partitionKey)
    ...
    .then((result) => updateItem(result.container, result.item)) //1.etag==2.etag
    .then((container) => updateItem(container, savedItem)) //2.etag != 1.etag
